...
type ServiceSortKey = 'id' | 'name' | 'duration' | 'price'
...
const Services = () => {
  ...
  const [sortKey, setSortKey] = useState<ServiceSortKey>('id')
  ...
  const requestSort = (key: ServiceSortKey) => {
  ...
  const getSortValue = (service: Service, key: ServiceSortKey): string | number => {
    switch (key) {
      case 'id':
        return service.id
      case 'name':
