...
const StatCard = ({ title, value, subtitle, growth, icon, colorClass }: StatCardProps) => (
...
const Reports = () => {
  ...
  const [ordersData, setOrdersData] = useState<OrdersReportItem[]>([])
  ...
  const totalOrders = ordersData.reduce((sum, d) => sum + (d.total || d.count || 0), 0)
  const completedOrders = ordersData.reduce((sum, d) => sum + (d.completed || 0), 0)
    ...
    return [
      {
        ...
        subtitle: comparison.previous ? `Previous: ${formatCurrency(comparison.previous.revenue || 0)}` : undefined,
      ...
      },
      {
        ...
        subtitle: comparison.previous ? `Previous: ${(comparison.previous.orders || 0).toLocaleString()}` : undefined,
      ...
      },
      {
        ...
        subtitle: comparison.previous?.completed ? `Previous: ${comparison.previous.completed.toLocaleString()}` : undefined,
