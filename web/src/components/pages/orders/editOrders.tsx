...
import type { Service, Vehicle, Customer, Staff, OrderStatus } from '../../../types'
...
const EditOrders = () => {
  ...
  const [_fetchingVehicles, setFetchingVehicles] = useState<boolean>(true)
  const [customers, setCustomers] = useState<Customer[]>([])
  const [_fetchingCustomers, setFetchingCustomers] = useState<boolean>(true)
  const [staffList, setStaffList] = useState<Staff[]>([])
  const [_fetchingStaff, setFetchingStaff] = useState<boolean>(true)
            ...
            <div className="w-full flex flex-row gap-5">
              <div className="w-full">
                <SearchableSelect
                  ...
                  getOptionLabel={(vehicle: Vehicle) => `${vehicle.name} - ${vehicle.plateNumber}`}
                  required
                />
              </div>
              <div className="w-full">
                <SearchableSelect
                  ...
                  getOptionLabel={(customer: Customer) => `${customer.name} (${customer.email})`}
                  required
                />
              </div>
              <div className="w-full">
                <SearchableSelect
                  ...
                  getOptionLabel={(staff: Staff) => `${staff.name}${staff.position ? ` - ${staff.position}` : ''}`}
                />
              </div>
            </div>
            <div>
                  ...
                  {cartItems.length === 0 ? (
                    ...
                    cartItems.map((item, index) => (
                      <tr key={index}>
                        ...
                        <td>
                          {item.empty ? '-' : Number(item.price).toFixed(2)}
                        </td>
                        <td>
                          {!item.empty && (
