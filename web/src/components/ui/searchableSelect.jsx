...
const SearchableSelect = ({ label, options, value, onChange, placeholder, getOptionLabel}) => {
  ...
  return (
    <div className='w-full relative' ref={ref}>
      ...
      <button
        type="button"
        className="w-full px-3 py-2 border border-gray-300 dark:border-gray-700 rounded-md focus:outline-none focus:ring-2 focus:ring-teal-500 dark:bg-gray-800 dark:text-white text-left flex justify-between items-center"
      ...
      {open && (
        <div className="absolute z-10 mt-1 w-full bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-700 rounded-md shadow-lg">
          <div className="p-2">
            <input
              type="text"
              className="w-full px-2 py-1 border border-gray-300 dark:border-gray-700 rounded-md focus:outline-none focus:ring-2 focus:ring-teal-500 dark:bg-gray-900 dark:text-white text-sm"
              placeholder="Search..."
              value={query}
              ...
              filtered.map((option) => (
                <button
                  ...
                  className={`w-full text-left px-3 py-2 text-sm hover:bg-gray-100 dark:hover:bg-gray-700 ${
                    String(option.id) === String(value) ? 'bg-teal-50 dark:bg-teal-950 text-teal-600' : 'text-gray-900 dark:text-white'
                  }`}
                  onClick={() => {
